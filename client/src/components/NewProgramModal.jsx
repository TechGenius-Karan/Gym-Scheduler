      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="mb-5">
          ...
          <input
            ...
            className="w-full bg-gray-800 border border-gray-700 focus:border-indigo-500 rounded-lg px-3 py-2.5 text-white text-sm outline-none transition placeholder:text-gray-600"
          />
        </div>
        <div className="mb-5">
          ...
          <div className="grid grid-cols-3 gap-2">
            {SOURCES.map(opt => (
              <button
                key={opt}
                onClick={() => { setSource(opt); setSelectedTemplateId(null); setSelectedTemplateData(null); setError(null) }}
                className={`py-2 px-3 rounded-lg text-sm font-medium border transition ${source === opt ? 'border-indigo-500 bg-indigo-500/10 text-white' : 'border-gray-700 text-gray-400 hover:border-gray-600 hover:text-white'}`}
              >
                {SOURCE_LABELS[opt]}
              </button>
            ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {templates.map(t => (
                  <button
                    ...
                    className={`text-left p-3 rounded-xl border text-sm transition disabled:opacity-50 ${selectedTemplateId === t.id ? 'border-indigo-500 bg-indigo-500/10' : 'border-gray-700 bg-gray-800 hover:border-gray-600'}`}
                  >
        ...
        <div className="flex gap-3 justify-end">
          ...
          <button onClick={handleCreate} disabled={!canCreate} className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-sm font-semibold rounded-lg transition">
